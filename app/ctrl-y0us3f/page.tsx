...
export default function AdminPage() {
  ...
  const handleIconUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const url = await handleFileUpload(file, 'images/');
      setFormData(prev => ({ ...prev, icon_emoji: url }));
      showToast('Icon image uploaded successfully', 'success');
    } catch (err: any) {
      setUploadProgress('');
      showToast(err.message || 'Error uploading icon image', 'error');
    }
  };
  ...
  const handleGalleryDragEnd = (result: DropResult) => {
    if (!result.destination) return;
    const items = Array.from(formData.gallery_urls || []);
    const [reorderedItem] = items.splice(result.source.index, 1);
    items.splice(result.destination.index, 0, reorderedItem);
    setFormData(prev => ({ ...prev, gallery_urls: items }));
  };
                        ...
                        {(provided, snapshot) => (
                          ...
                          >
                            <div className="flex items-center gap-4 flex-1">
                              ...
                              {project.icon_emoji ? (
                                project.icon_emoji.startsWith('http') || project.icon_emoji.startsWith('/') ? (
                                  <img src={project.icon_emoji} alt="" className="w-8 h-8 rounded-lg object-cover" />
                                ) : (
            ...
            <div className="flex-1 overflow-y-auto p-6 md:border-r border-white/10">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-400 mb-1">Project Icon (Emoji or Image URL / Upload)</label>
                    <div className="flex items-center gap-2">
                      <input
                        type="text"
                        value={formData.icon_emoji || ''}
                        onChange={e => setFormData({...formData, icon_emoji: e.target.value})}
                        placeholder="Emoji or Image URL (leave empty to show no icon)"
                      ...
                      />
                      <label className="cursor-pointer bg-white/5 hover:bg-white/10 border border-white/10 px-3 py-2 rounded-lg text-xs font-medium text-slate-300 flex items-center gap-1.5 shrink-0 transition-colors">
                ...
                <div className="space-y-3 p-3 bg-surface-low/50 border border-white/5 rounded-lg">
                  ...
                  {/* Gallery Images Section (Drag-and-Drop Reorderable) */}
                  <div className="pt-2 border-t border-white/5 mt-3">
                    <div className="flex items-center justify-between mb-2">
                      <label className="block text-xs font-medium text-slate-400">Gallery Images</label>
                      <span className="text-[10px] text-slate-500">Drag thumbnails with cursor to reorder</span>
                    </div>

                    <DragDropContext onDragEnd={handleGalleryDragEnd}>
                      <Droppable droppableId="gallery-images-droppable" direction="horizontal">
                        {(provided) => (
                          <div 
                            ref={provided.innerRef} 
                            {...provided.droppableProps}
                            className="flex flex-wrap gap-2.5 mb-2 items-center"
                          >
                            {(formData.gallery_urls || []).map((url, i) => (
                              <Draggable key={`${url}-${i}`} draggableId={`gallery-img-${i}`} index={i}>
                                {(provided, snapshot) => (
                                  <div
                                    ref={provided.innerRef}
                                    {...provided.draggableProps}
                                    {...provided.dragHandleProps}
                                    className={`relative w-20 h-20 rounded-lg overflow-hidden bg-surface border transition-all cursor-grab active:cursor-grabbing group ${
                                      snapshot.isDragging 
                                        ? 'border-accent-purple ring-2 ring-accent-purple/50 scale-110 z-50 shadow-2xl' 
                                        : 'border-white/10 hover:border-white/30'
                                    }`}
                                  >
                                    <img src={url} alt={`Gallery ${i}`} className="w-full h-full object-cover pointer-events-none" />
                                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                      <button 
                                        type="button"
                                        onClick={() => removeGalleryImage(i)}
                                        className="bg-red-500/80 hover:bg-red-600 rounded-md p-1 transition-colors"
                                        title="Remove image"
                                      >
                                        <X className="w-3.5 h-3.5 text-white" />
                                      </button>
                                    </div>
                                    <span className="absolute bottom-1 left-1 bg-black/70 backdrop-blur-md px-1.5 py-0.5 rounded text-[9px] font-bold text-slate-300 pointer-events-none">
                                      #{i + 1}
                                    </span>
                                  </div>
                                )}
                            ...
                            <label className="w-20 h-20 rounded-lg border-2 border-dashed border-white/20 hover:border-accent-purple/50 flex flex-col items-center justify-center cursor-pointer bg-white/5 hover:bg-accent-purple/10 transition-all shrink-0">
              ...
              <div className="w-full max-w-[280px] bg-surface rounded-2xl border border-white/10 overflow-hidden shadow-xl">
                ...
                <div className="p-4">
                  <div className="flex items-center gap-2 mb-2">
                    {formData.icon_emoji ? (
                      formData.icon_emoji.startsWith('http') || formData.icon_emoji.startsWith('/') ? (
                        <img src={formData.icon_emoji} alt="" className="w-5 h-5 rounded object-cover" />
                      ) : (
